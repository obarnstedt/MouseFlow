...
def mouseflow(vid_dir=os.getcwd(), dlc_dir='', facekey='', bodykey='', dgp=True, batch=True, 
              overwrite=False, of_type='farneback', filetype='.mp4', vid_output=1000,
              bodyflip=False, faceflip=False,
              ...
              faceregion_conf_thresh = .99, faceregion_size_whiskers=1, faceregion_size_nose=1.5, 
    ...
    # facefiles = [f for f in facefiles if '_flipped.mp4' not in f]
    # bodyfiles = [b for b in bodyfiles if '_flipped.mp4' not in b]
    ...
    #     #  FACE ANALYSIS
    #     faceDLC = glob.glob(os.path.join(dir_out, os.path.basename(facefile)[:-4] + '*000.h5'))[0]
    ...
    #     frame_diff = np.nan#face_processing.facemotion(facefile, facemasks)#, round(faceregion_smooth_window * FaceCam_FPS))
...
def runMF(dlc_dir=os.getcwd(),
    ...
    for faceDLC in facefiles:
        ...
        faceDLC_analysis = faceDLC[:-3] + '_analysis.h5'
        print('Processing DLC data from '+faceDLC)
        facefile = glob.glob(os.path.join(os.path.dirname(dlc_dir), os.path.basename(faceDLC).split('DeepCut')[0]+'*'))[0]
        facevidcap = cv2.VideoCapture(facefile)
        FaceCam_FPS = facevidcap.get(cv2.CAP_PROP_FPS)
